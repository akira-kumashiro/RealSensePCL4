...
bool RealSenseProcessor::run(void)
{
	...
	if (sts >= Status::PXC_STATUS_NO_ERROR)
	{
		...
		{
			/*pcl::PointCloud<pcl::PointXYZRGBA>::Ptr filtered_hand(new pcl::PointCloud<pcl::PointXYZRGBA>);
			pcl::PointCloud<pcl::PointXYZRGBA>::Ptr filtered_camera(new pcl::PointCloud<pcl::PointXYZRGBA>);

			filtered_camera->width = DEPTH_WIDTH;
			filtered_camera->height = DEPTH_HEIGHT;
			...
			if (sample)
			{
				...
				if (handData)
				{
					...
					/*if (enableBilateral)
					{
						bilateralH.filter(*filtered_hand);
						viewer->updatePointCloud(filtered_hand, "handcloud");
					}
					else*/
				...
				/*if (enableBilateral)
				{
					bilateralC.filter(*filtered_camera);
					viewer->updatePointCloud(filtered_camera, "cameracloud");
	...
	if (isDepthImage)
	{
		...
		for (int y = 0; y < rawDepthImage.rows; y++)
		{
			...
			for (int x = 0; x < rawDepthImage.cols; x++)
			{
				...
				{
					/*int tmp = rawDepthDiffImagePtr[x];
					tmp -= 32;
					if (tmp < 0)
					tmp = 0;
					rawDepthDiffImagePtr[x] = tmp;*/
					rawDepthDiffImagePtr[x] = 0;
				}
			}
		}
		//detectContours();
		//cv::imshow("diff", rawDepthDiffImage);
	...
	if (handData != nullptr&&handData)
	{
		...
		for (int i = 0; i < numberOfHands; i++)
		{
			...
			for (int j = 0; j < PXCHandData::NUMBER_OF_JOINTS; j++)
			{
				...
				/*point.x = jointData.positionWorld.x / CLOUD_SCALE;
				point.y = jointData.positionWorld.y / CLOUD_SCALE;
				point.z = jointData.positionWorld.z / CLOUD_SCALE;*/
